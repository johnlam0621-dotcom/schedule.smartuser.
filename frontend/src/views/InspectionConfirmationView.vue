  ...
  <main class="confirmation-page">
    <section class="confirmation-header">
      <div>
        ...
        <p>Search by MACID or phone number to review the inspection photos and videos.</p>
      </div>
    </section>
    ...
    <form class="confirmation-search" @submit.prevent="applySearch">
      ...
      <label>Search phone number
        <input v-model.trim="searchPhone" type="search" inputmode="tel" placeholder="Enter phone number" />
      </label>
      <button type="submit">Search</button>
      <button v-if="searchApplied" type="button" class="secondary" @click="clearSearch">Clear</button>
    </form>

    <div v-if="loading" class="confirmation-empty">Searching inspections…</div>
    <div v-else-if="!searchApplied" class="confirmation-empty">Enter a MACID or phone number, then click Search.</div>
    <div v-else-if="!jobs.length" class="confirmation-empty">No matching inspections were found.</div>
    <div v-else class="search-result-count">{{ jobs.length }} matching inspection(s)</div>

    <section v-if="jobs.length" class="inspector-group">
      <article v-for="job in jobs" :key="job.id" class="confirmation-card" :class="{ confirmed: job.confirmed }">
        <header class="job-heading">
          <div>
            ...
            <p>Phone: {{ job.phoneNumber || '—' }} · Inspector: {{ job.inspector || '—' }}</p>
            <p>{{ job.address || 'No address recorded' }}</p>
          </div>
          <div class="job-state">
            <span class="status-pill" :class="job.fieldStatus">{{ statusLabel(job.fieldStatus) }}</span>
            <span v-if="job.confirmed" class="confirmed-pill">✓ Inspection done</span>
          </div>
        </header>
        ...
        <div class="review-row">
          <div v-if="job.confirmed" class="confirmation-audit">
            Marked done by {{ job.confirmedBy || 'reviewer' }}<span v-if="job.confirmedAt"> · {{ formatDateTime(job.confirmedAt) }}</span>
          </div>
          <button v-if="job.confirmed" type="button" class="secondary reopen" :disabled="saving[job.id]" @click="setConfirmed(job, false)">Reopen</button>
          <button v-else type="button" :disabled="saving[job.id] || !isFinal(job)" :title="isFinal(job) ? '' : 'Inspector must mark the job Done or Unavailable first'" @click="setConfirmed(job, true)">
            {{ saving[job.id] ? 'Saving…' : 'Inspection done' }}
          </button>
        </div>

        <section class="job-media">
          ...
          <div v-else class="media-grid">
            ...
            <article v-for="item in videoFiles(job)" :key="item.file.id" class="media-card video">
              <button type="button" class="video-preview" @click="openMedia(item, true)">
                <span class="media-thumb">▶</span><strong>{{ item.label }}</strong><small>{{ item.file.name }}</small>
                <em v-if="item.file.remark">{{ item.file.remark }}</em>
              </button>
              <a class="media-download" :href="downloadUrl(item.file)" download>Download video</a>
            </article>
          </div>
        </section>
      </article>
    ...
    <div v-if="activeMedia" class="confirmation-modal" role="dialog" aria-modal="true" @click.self="closeMedia">
      <section>
        ...
        <p v-if="mediaLoading">Preparing video…</p>
        <video v-else-if="activeMedia.video" :src="activeMedia.url" controls autoplay preload="metadata" @error="mediaPlaybackFailed"></video>
        <img v-else :src="activeMedia.url" :alt="activeMedia.label" />
        <footer>
          <strong>{{ activeMedia.label }}</strong><span>{{ activeMedia.name }}</span>
          <a v-if="activeMedia.video && activeMedia.url" class="modal-download" :href="`${activeMedia.url}?download=true`" download>Download smaller video</a>
          <a v-if="activeMedia.video" class="modal-download" :href="activeMedia.downloadUrl" download>Download original</a>
        </footer>
      </section>
    </div>
  </main>
...
async function applySearch() {
  ...
  if (!searchMacId.value && !searchPhone.value) {
    searchApplied.value = false
    error.value = 'Enter a MACID or phone number before searching.'
    return
  }
  loading.value = true
  clearObjectUrls()
  try {
    const response = await http.get('/inspector/inspection-done/search', {
      params: { macId: searchMacId.value || undefined, phone: searchPhone.value || undefined }
    })
    jobs.value = response.jobs || []
    searchApplied.value = true
    await loadPhotoPreviews(jobs.value)
  } catch (requestError) {
    jobs.value = []
    searchApplied.value = true
    error.value = requestError.message || 'The inspection search failed.'
  } finally { loading.value = false }
}
function clearSearch() {
  searchMacId.value = ''; searchPhone.value = ''; jobs.value = []
  searchApplied.value = false; error.value = ''; notice.value = ''; clearObjectUrls()
}
...
async function setConfirmed(job, confirmed) {
  ...
  try {
    ...
    notice.value = confirmed ? 'Inspection marked done.' : 'Inspection reopened.'
...
async function loadPhotoPreviews(results) {
  await Promise.all(results.flatMap((job) => photoFiles(job)).map(async ({ file }) => {
...
async function openMedia(item, video) {
  ...
  activeMedia.value = {
    ...
    name: item.file.name,
    video,
    url: video ? '' : previews[item.file.id],
    downloadUrl: video ? downloadUrl(item.file) : ''
  }
  if (!video) return
  try {
    activeMedia.value.url = await prepareVideoPlayback(item.file)
  } catch (requestError) {
...
function mediaPlaybackFailed() {
  error.value = 'The video could not be played in this browser. Use Download video to save the original file.'
}
function closeMedia() { activeMedia.value = null; mediaLoading.value = false }
...
.video-preview{padding:0!important;background:transparent!important;color:#17283d!important;text-align:left;display:grid;gap:5px}.media-download,.modal-download{display:block;padding:8px 10px;border-radius:6px;background:#e8f2ff;color:#075da8;text-align:center;font-size:12px;font-weight:700;text-decoration:none}.modal-download{margin-top:8px;width:max-content}
</style>
